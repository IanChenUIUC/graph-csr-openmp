 ...
 */
template <class I, class FE> inline I non_adjacent_find(I ib, I ie, FE fe)
{
    if (ib == ie)
        return ie;
 ...
 * @returns iterator to the first non-adjacent element
 */
template <class I> inline auto non_adjacent_find(I ib, I ie)
{
 ...
 */
template <class I, class M> inline void value_index(I ib, I ie, M &a)
{
    size_t i = 0;
    for (; ib != ie; ++ib)
        a[*ib] = i++;
 ...
 */
template <class IX, class IA, class FE> inline IA unique_last_copy(IX xb, IX xe, IA ab, FE fe)
{
    if (xb == xe)
        return ab;
 ...
 * @returns iterator to the end of output
 */
template <class IX, class IA> inline IA unique_last_copy(IX xb, IX xe, IA ab)
{
    auto fe = [](const auto &a, const auto &b) { return a == b; };
 ...
 * @returns iterator to the end of updated input
 */
template <class IX, class IY> inline IX set_difference_inplace(IX xb, IX xe, IY yb, IY ye)
{
...
inline IX set_union_last_inplace(IX xb, IX xe, IY yb, IY ye, IB bb, IB be, FL fl, FE fe)
{
    ...
    if (yb == ye)
        return xe;
    if (xb == xe)
        return unique_last_copy(yb, ye, xb);
 ...
 * @returns iterator to the end of updated input
 */
template <class IX, class IY, class IB> inline auto set_union_last_inplace(IX xb, IX xe, IY yb, IY ye, IB bb, IB be)
{
    auto fl = [](const auto &a, const auto &b) { return a < b; };
    auto fe = [](const auto &a, const auto &b) { return a == b; };
