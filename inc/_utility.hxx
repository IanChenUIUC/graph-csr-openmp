 ...
 * @tparam V value type
 */
template <class K, class V> struct PairFirst
{
    inline K &operator()(pair<K, V> &x) noexcept
    {
        return x.first;
    }
};
 ...
 * @tparam V value type
 */
template <class K, class V> struct PairSecond
{
    inline V &operator()(pair<K, V> &x) noexcept
    {
        return x.second;
    }
};
 ...
 * @tparam V value type
 */
template <class K, class V> struct ConstPairFirst
{
    inline const K &operator()(const pair<K, V> &x) noexcept
    {
        return x.first;
    }
};
 ...
 * @tparam V value type
 */
template <class K, class V> struct ConstPairSecond
{
    inline const V &operator()(const pair<K, V> &x) noexcept
    {
        return x.second;
    }
};
 ...
 * @tparam V value type
 */
template <class K, class V> struct PairFirstValue
{
    inline K operator()(const pair<K, V> &x) noexcept
    {
        return x.first;
    }
};
 ...
 * @tparam V value type
 */
template <class K, class V> struct PairSecondValue
{
    inline V operator()(const pair<K, V> &x) noexcept
    {
        return x.second;
    }
};
 ...
 */
template <class T> inline float duration(const T &start, const T &stop)
{
 ...
 * @returns time duration in milliseconds
 */
template <class T> inline float duration(const T &start)
{
    auto stop = timeNow();
 ...
 */
template <class F> inline float measureDuration(F fn, int N = 1)
{
    auto start = timeNow();
    for (int i = 0; i < N; ++i)
        fn();
 ...
 * @returns average duration in milliseconds
 */
template <class F> inline float measureDurationMpi(F fn, int N = 1)
{
    double total = 0;
    for (int i = 0; i < N; ++i)
    {
 ...
 * @returns average duration in milliseconds
 */
template <class F> inline float measureDurationMarked(F fn, int N = 1)
{
    float total = 0;
    for (int i = 0; i < N; ++i)
        fn([&](auto fm) {
 ...
 * @returns average duration in milliseconds
 */
template <class F> inline float measureDurationMarkedMpi(F fn, int N = 1)
{
    float total = 0;
    for (int i = 0; i < N; ++i)
        fn([&](auto fm) {
 ...
 */
template <class F> inline bool retry(F fn, int N = 2)
{
    for (int i = 0; i < N; ++i)
        if (fn())
            return true;
 ...
 */
#define CMOVE_VECTOR(t, f) CMOVE(!(t).empty(), t, f)
 ...
 * @returns move(t) if t is not empty, otherwise f
 */
#define CMOVE_GRAPH(t, f) CMOVE((t).order() > 0, t, f)
#pragma endregion
