...
inline size_t readMtxFormatHeaderW(bool &symmetric, size_t &rows, size_t &cols, size_t &size, string_view data)
{
    ...
    for (; it != ie; it = findNextLine(it, ie))
    {
        if (*it != '%')
            break;
        if (data.substr(it - ib, 14) != "%%MatrixMarket")
            continue;
    ...
    if (h1 != "matrix" || h2 != "coordinate")
        throw FormatError("Invalid MTX header (unknown format)", ib);
...
inline void readEdgelistFormatDoChecked(string_view data, bool symmetric, FB fb)
{
    ...
    for (; it != ie; it = findNextLine(it, ie))
    {
        ...
        if (u < 0 || v < 0)
            throw FormatError("Invalid Edgelist body (negative vertex-id)", il);
        fb(u, v, w);
        if (symmetric && u != v)
            fb(v, u, w);
    }
...
{
    if constexpr (CHECK)
        readEdgelistFormatDoChecked<WEIGHTED, BASE>(data, symmetric, fb);
    else
        readEdgelistFormatDoUnchecked<WEIGHTED, BASE>(data, symmetric, fb);
}
...
inline string_view readEdgelistFormatBlock(string_view data, size_t b, size_t B)
{
    ...
    if (bb != db && !isNewline(*bb - 1))
        bb = findNextLine(bb, de);
    if (be != db && !isNewline(*be - 1))
        be = findNextLine(be, de);
...
inline vector<size_t> readEdgelistFormatToListsOmpU(IIK degrees, IIK sources, IIK targets, IIE weights,
                                                    string_view data, bool symmetric)
{
...
#pragma omp atomic
            ...
            if constexpr (CHECK)
            {
                try
                {
                    readEdgelistFormatDo<WEIGHTED, BASE, true>(bdata, symmetric, fb);
                }
                ...
                readEdgelistFormatDo<WEIGHTED, BASE>(bdata, symmetric, fb);
        }
...
inline void convertEdgelistToCsrListsW(IO offsets, IK edgeKeys, IE edgeValues, IK degrees, IK sources, IK targets,
                                       IE weights, size_t rows)
{
...
inline void convertEdgelistToCsrListsOmpW(IO offsets, IK edgeKeys, IE edgeValues, IIO poffsets, IIK pedgeKeys,
                                          IIE pedgeValues, IIK degrees, IIK sources, IIK targets, IIE weights,
                                          const vector<size_t> &counts, size_t rows)
{
...
#pragma omp parallel for schedule(static, 2048)
    for (size_t u = 0; u < rows; ++u)
    {
        if (PARTITIONS == 1)
        {
            ...
            degrees[0][u] += degrees[1][u];
        else if (PARTITIONS == 4)
            degrees[0][u] += degrees[1][u] + degrees[2][u] + degrees[3][u];
        else if (PARTITIONS == 8)
            degrees[0][u] += degrees[1][u] + degrees[2][u] + degrees[3][u] + degrees[4][u] + degrees[5][u] +
                             degrees[6][u] + degrees[7][u];
        else
        {
