 ...
 */
template <class K = uint32_t, class V = None, class E = None> class DiGraph
{
#pragma region TYPES
  public:
    /** Key type (vertex id). */
...
#pragma region FOREACH
  public:
     ...
     */
    template <class FP> inline void forEachVertex(FP fp) const noexcept
    {
        for (K u = 0; u < span(); ++u)
            if (exists[u])
                fp(u, values[u]);
     ...
     */
    template <class FP> inline void forEachVertexKey(FP fp) const noexcept
    {
        for (K u = 0; u < span(); ++u)
            if (exists[u])
                fp(u);
...
#pragma region UPDATE
  public:
     ...
     */
    template <class FT> inline void addEdgeIf(K u, K v, E w, FT ft)
    {
        addVertex(u);
        addVertex(v);
        if (ft(u))
     ...
     * @param ft test function (source vertex id)
     */
    template <class FT> inline void removeEdgeIf(K u, K v, FT ft)
    {
        if (!hasVertex(u) || !hasVertex(v))
            return;
        if (ft(u))
 ...
 * @param x graph
 */
template <class G> inline void writeGraphSizes(ostream &a, const G &x)
{
 ...
 * @param x graph
 */
template <class G> inline void writeGraphDetailed(ostream &a, const G &x)
{
    a << "order: " << x.order() << " size: " << x.size();
    a << (x.directed() ? " [directed]" : " [undirected]") << " {\n";
    x.forEachVertex([&](auto u, auto d) {
        a << u << ":" << d << " ->";
        x.forEachEdge(u, [&](auto v, auto w) { a << " " << v << ":" << w; });
        a << "\n";
    });
    a << "}";
}
 ...
 */
template <class G> inline void writeGraph(ostream &a, const G &x, bool detailed = false)
{
    if (detailed)
        writeGraphDetailed(a, x);
    else
        writeGraphSizes(a, x);
}
 ...
 * @param detailed write detailed information?
 */
template <class K, class V, class E> inline void write(ostream &a, const DiGraph<K, V, E> &x, bool detailed = false)
{
 ...
 * @param x graph
 */
template <class K, class V, class E> inline ostream &operator<<(ostream &a, const DiGraph<K, V, E> &x)
{
    write(a, x);
    return a;
}
 ...
 * @param x csr graph
 */
template <class K, class V, class E, class O> inline ostream &operator<<(ostream &a, const DiGraphCsr<K, V, E, O> &x)
{
    write(a, x);
    return a;
}
