 ...
 */
template <char EOL = '\n', class I> inline I findNextLine(I ib, I ie)
{
    for (; ib < ie; ++ib)
        if (*ib == EOL)
            return ++ib;
 ...
 */
template <class I, class FW> inline I findNextWhitespace(I ib, I ie, FW fw)
{
    for (; ib < ie; ++ib)
        if (isBlank(*ib) || isNewline(*ib) || fw(*ib))
            return ib;
    return ie;
}
 ...
 */
template <class I, class FW> inline I findNextNonWhitespace(I ib, I ie, FW fw)
{
    for (; ib < ie; ++ib)
        if (!isBlank(*ib) && !isNewline(*ib) && !fw(*ib))
            return ib;
 ...
 */
template <class I, class FU> inline I findNextBlank(I ib, I ie, FU fu)
{
    for (; ib < ie; ++ib)
        if (isBlank(*ib) || fu(*ib))
            return ib;
 ...
 */
template <class I, class FU> inline I findNextNonBlank(I ib, I ie, FU fu)
{
    for (; ib < ie; ++ib)
        if (!isBlank(*ib) && !fu(*ib))
            return ib;
 ...
 */
template <class I> inline I findNextDigit(I ib, I ie)
{
 ...
 */
template <class I> inline I findNextNonDigit(I ib, I ie)
{
    for (; ib != ie && isDigit(*ib); ++ib)
        ;
    return ib;
 ...
 */
template <class I, class FU, class FW> inline pair<I, I> findNextToken(I ib, I ie, FU fu, FW fw)
{
 ...
 */
template <int FULL = 0, class T, class I> inline I parseWholeNumberW(T &a, I ib, I ie)
{
    a = T();
 ...
 * @returns iterator to end of number, or error if FULL==2
 */
template <int FULL = 0, class T, class I> inline I parseIntegerW(T &a, I ib, I ie)
{
    // Skip if empty.
    if (ib == ie)
        return ib;
 ...
 * @returns iterator to end of number, or error if FULL==2
 */
template <int FULL = 0, class T, class I> inline I parseFloatW(T &a, I ib, I ie)
{
    // Skip if empty.
    if (ib == ie)
        return ib;
    ...
    {
        I id = ++ib;
        ib = parseWholeNumberW(v, ib, ie);
        d = int(ib - id);
    }
    if (ib != ie && (*ib == 'e' || *ib == 'E'))
        ib = parseIntegerW<FULL>(e, ib + 1, ie);
 ...
 * @returns iterator to end of number, or error if FULL==2
 */
template <int FULL = 0, class T, class I> inline I parseNumberW(T &a, I ib, I ie)
{
    if constexpr (is_integral<T>::value)
        return parseIntegerW<FULL>(a, ib, ie);
    else if constexpr (is_floating_point<T>::value)
        return parseFloatW<FULL>(a, ib, ie);
    return ib;
}
#pragma endregion
 ...
 */
template <bool CHECK = false, class I, class FU, class FW> inline I readTokenW(string_view &a, I ib, I ie, FU fu, FW fw)
{
    auto [tb, te] = findNextToken(ib, ie, fu, fw);
    if constexpr (CHECK)
    {
        if (tb == te)
            throw FormatError("Failed to read token (empty)", tb);
    }
 ...
 */
template <bool CHECK = false, class T, class I, class FU, class FW> inline I readNumberW(T &a, I ib, I ie, FU fu, FW fw)
{
    auto [tb, te] = findNextToken(ib, ie, fu, fw);
    if constexpr (CHECK)
    {
        if (tb == te)
            throw FormatError("Failed to read number (empty)", tb);
    }
    tb = parseNumberW<CHECK ? 2 : 1>(a, tb, te);
    if constexpr (CHECK)
    {
        if (tb != te)
            throw FormatError("Failed to read number (bad format)", tb);
    }
    return te;
}
#pragma endregion
