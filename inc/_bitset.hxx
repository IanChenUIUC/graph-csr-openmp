 ...
 * @tparam V value type
 */
template <class K = uint32_t, class V = NONE> class LazyBitset
{
#pragma region TYPES
  public:
    /** The key type. */
...
#pragma region FOREACH
  public:
     ...
     */
    template <class F> inline void forEach(F fp) const noexcept
    {
     ...
     */
    template <class F> inline void forEachKey(F fp) const noexcept
    {
        for (const auto &p : pairs)
            fp(p.first);
    }
 ...
 * @param x bitset
 */
template <class B> void writeBitset(ostream &a, const B &x)
{
    a << "{\n";
    x.forEach([&](auto k, auto v) { a << "  " << k << ": " << v << "\n"; });
    a << "}";
}
 ...
 * @param x bitset
 */
template <class K, class V> inline void write(ostream &a, const LazyBitset<K, V> &x)
{
    writeBitset(a, x);
}
 ...
 * @param x bitset
 */
template <class K, class V> inline ostream &operator<<(ostream &a, const LazyBitset<K, V> &x)
{
    write(a, x);
    return a;
}
