...
int main(int argc, char **argv)
{
    ...
    float t = measureDuration([&]() {
        if (weighted)
            counts = readEdgelistFormatToListsOmpU<true, 1, false, NUM_PARTITIONS>(
                degrees.data(), sources.data(), targets.data(), weights.data(), data, symmetric);
        else
            counts = readEdgelistFormatToListsOmpU<false, 1, false, NUM_PARTITIONS>(
                degrees.data(), sources.data(), targets.data(), weights.data(), data, symmetric);
        convertEdgelistToCsrListsOmpW<false, NUM_PARTITIONS>(
            xc.offsets.data(), xc.edgeKeys.data(), xc.edgeValues.data(), poffsets.data(), pedgeKeys.data(),
            pedgeValues.data(), degrees.data(), sources.data(), targets.data(), weights.data(), counts, rows);
    });
